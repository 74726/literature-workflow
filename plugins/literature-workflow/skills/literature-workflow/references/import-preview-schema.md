...
Required top-level fields: `schema`, `preview_id`, `created_at`, `source_manifest`, `source_fingerprint`, `preview_status`, `confirmation_required`, `target_collections`, `rows`, `summary`, and `index_plan`.
...
Each deduplicated row has one primary action: `create_item`, `update_existing`, `no_op`, or `blocked`. Secondary actions are `enrich_metadata`, `attach_linked_pdf`, `add_to_collection`, and an explicitly authorized `replace_broken_attachment`.

Primary-action counts must sum to the deduplicated row count. Report metadata completeness, PDF validity, permanent-path validity, and collection distribution separately.
